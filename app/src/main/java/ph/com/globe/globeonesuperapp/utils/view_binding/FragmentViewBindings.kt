...
abstract class ViewBindingFragment<T : ViewBinding>(
    ...
    private val bottomNavVisibility: BottomNavVisibility?
) :
    androidx.fragment.app.Fragment(), HasLogTag {
...
/**
 * [ViewBindingFragment] that never displays the bottom navigation view.
 */
abstract class NoBottomNavViewBindingFragment<T : ViewBinding>(
    bindViewBy: bindViewBy<T>
) : ViewBindingFragment<T>(bindViewBy, BottomNavVisibility.NO_BOTTOM_NAV)

/**
 * [ViewBindingFragment] that displays the bottom navigation view only if the user
 * is authenticated.
 */
abstract class AuthenticatedBottomNavViewBindingFragment<T : ViewBinding>(
    bindViewBy: bindViewBy<T>
) : ViewBindingFragment<T>(bindViewBy, BottomNavVisibility.AUTHENTICATED_BOTTOM_NAV)

/**
 * [ViewBindingFragment] that always displays the bottom navigation view.
 */
abstract class BottomNavViewBindingFragment<T : ViewBinding>(
    bindViewBy: bindViewBy<T>
) : ViewBindingFragment<T>(bindViewBy, BottomNavVisibility.VISIBLE_BOTTOM_NAV)

/**
 * [ViewBindingFragment] that doesn't control the hiding and displaying of bottom
 * navigation view. All of the fragments that are nested within other fragments that
 * already control the bottom navigation displaying should inherit this class.
 */
abstract class NestedViewBindingFragment<T : ViewBinding>(
    bindViewBy: bindViewBy<T>
) : ViewBindingFragment<T>(bindViewBy, null)

/**
 * This enum class defines possible bottom navigation visibility.
 */
enum class BottomNavVisibility {
    /**
     * The fragment has no bottom navigation displayed.
     */
    NO_BOTTOM_NAV,

    /**
     * The fragment will have bottom nav displayed when the user is authenticated.
     */
    AUTHENTICATED_BOTTOM_NAV,

    /**
     * The fragment will always have bottom nav displayed.
     */
    VISIBLE_BOTTOM_NAV
}
