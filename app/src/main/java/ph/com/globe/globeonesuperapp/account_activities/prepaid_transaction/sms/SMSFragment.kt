package ph.com.globe.globeonesuperapp.account_activities.prepaid_transaction.sms
...
@AndroidEntryPoint
class SMSFragment : NestedViewBindingFragment<ScreenSliderFragmentBinding>({
    ...
    private val smsViewModel: SMSViewModel by hiltNavGraphViewModels(R.id.account_activities_subgraph)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(viewBinding) {
            accountActivityViewModel.enrolledAccount.observe(viewLifecycleOwner) {
                if (it != null) {
                    smsViewModel.setEnrolledAccount(it)
                }
            }
            ...
            adapter = PrepaidLedgerAdapter(
                ...
                reachEnd = { smsViewModel.loadMore() },
                somethingWentWrongOnClick = { smsViewModel.reload() },
                backToTopOnCLick = {
            ...
            prepaidLedgerViewModel.dateFilter.observe(viewLifecycleOwner) {
                smsViewModel.setFilterDate(it)
            }

            smsViewModel.transactions.observe(viewLifecycleOwner) {
                adapter.submitList(it)
            }
        }
    }

    override val logTag = "SMSFragment"
}
