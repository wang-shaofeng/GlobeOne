 ...
 */
class RequestTapToRevealBubbleTimer @Inject constructor() {
    private var timer: CountDownTimer? = null

    private fun createNewTimer(receiver: RequestTapToRevealBubbleReceiver) =
        object : CountDownTimer(COUNTDOWN_PERIOD, COUNTDOWN_UPDATE_INTERVAL) {

            override fun onFinish() {
                receiver.countDownTapToRevealFinished()
            }

            override fun onTick(timeRemaining: Long) = Unit
        }

    fun startCountDown(receiver: RequestTapToRevealBubbleReceiver) {
        timer?.cancel()
...
interface RequestTapToRevealBubbleReceiver {

    fun countDownTapToRevealFinished()
}
